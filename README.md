...
현재 화면 데이터와 AI 처리는 상품 감각을 검증하기 위한 로컬 시뮬레이션입니다. 네 핵심 화면은 앱 도메인·Repository 스트림에 연결됐고 Supabase 로컬 스키마도 준비됐지만, 영구 저장, STT, E1~E7 서버 파이프라인은 아직 연결되지 않았습니다.
...
- 화면의 꿈·원고 콘텐츠와 진척은 `MemoryRepository`에서 공급되며 변경 사항이 화면에 반영됩니다. 현재 구현은 메모리 기반이므로 앱 재시작 시 저장되지 않습니다.
