...
현재 화면 데이터와 AI 처리는 상품 감각을 검증하기 위한 로컬 시뮬레이션입니다. Supabase 로컬 스키마와 앱 도메인·Repository 계약은 준비됐지만 화면 배선, 영구 저장, STT, E1~E7 서버 파이프라인은 아직 연결되지 않았습니다.
...
| `lib/domain/` | 불변 도메인 모델, Repository 계약, DB 직렬화, MU·명료도 순수 규칙 |
| `lib/data/` | 현재 프로토타입 데이터를 담은 `MemoryRepository`; 영구 저장 구현은 후속 WO |
| `lib/di/` | Riverpod 기반 Repository 주입 경계 |
| `lib/ui/` | 화면별 Flutter UI |
| `test/ui/` | 핵심 화면 위젯 테스트 |
| `test/contract/` | 저장소 구현체가 공통으로 통과해야 하는 계약 테스트 |
...
- 화면의 꿈·원고 콘텐츠는 아직 위젯에 하드코딩되어 있습니다. 동일한 프로토타입 데이터가 `MemoryRepository`에 준비됐지만 화면 배선은 다음 WO 범위이며, 앱 재시작 시 저장되지 않습니다.
- 로컬 Supabase에는 핵심 스키마·시드, 전 테이블 RLS와 트랜잭션 RPC가 있습니다. 앱 화면은 아직 이 데이터에 연결되지 않았고 Apple 인증, AI 모델, 푸시, 앱 잠금, PDF 출력도 구현 전입니다.
