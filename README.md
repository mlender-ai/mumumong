...
현재 화면 데이터와 AI 처리는 상품 감각을 검증하기 위한 로컬 시뮬레이션입니다. Supabase 로컬 스키마와 앱 도메인 계약은 준비됐지만 화면 저장소, STT, E1~E7 서버 파이프라인은 아직 연결되지 않았습니다.
...
| `lib/domain/` | 불변 도메인 모델, DB 직렬화, MU·명료도 순수 규칙 |
| `lib/data/` | 후속 WO에서 구현할 저장소·데이터 계층 |
...
- 화면의 꿈·원고 콘텐츠는 아직 하드코딩되어 있습니다. 도메인 모델과 직렬화는 구현됐지만 저장소 연결 전이며, 진행 카운터는 실행 중 임시 상태로 유지됩니다.
